...
def get_namespace_info(namespace):
    try:
        env = os.environ.copy()
        pods = run_kubectl(
            ["kubectl", "get", "pods", "-n", namespace, "-o", "json"], env=env
        )
        deployments = run_kubectl(
            ["kubectl", "get", "deployments", "-n", namespace, "-o", "json"], env=env
        )
        ...
        running_pods = sum(
            1 for pod in pod_data["items"] if pod["status"]["phase"] == "Running"
        )
        ...
        for deploy in deployment_data["items"]:
            ...
            print(
                f"  Replicas: {deploy['status'].get('replicas', 0)}/{deploy['status'].get('availableReplicas', 0)}"
            )
