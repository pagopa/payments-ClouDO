...
export default function AuditPage() {
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'manual' | 'api' | 'action'>('all');
  const [actionTypeFilter, setActionTypeFilter] = useState<'all' | 'mutation' | 'destruction'>('all');
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter(log => {
      ...
      const matchesFilter =
        activeFilter === 'all' ||
        (activeFilter === 'manual' && (log.operator !== 'api' && log.operator !== 'azure-action')) ||
        (activeFilter === 'api' && log.operator === 'api') ||
        (activeFilter === 'action' && log.operator === 'azure-action');

      const isMutation = log.action?.includes('CREATE') || log.action?.includes('UPSERT');
      ...
      return matchesSearch && matchesFilter && matchesActionType && matchesFrom && matchesTo;
    });
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      create: baseFiltered.filter(l => l.action?.includes('CREATE') || l.action?.includes('UPSERT')).length,
        ...
        <div className="flex items-center gap-2 px-8 pb-4">
          ...
          <div className="flex items-center gap-4 bg-cloudo-dark/30 px-3 py-1 border border-cloudo-border/50">
            <div className="flex items-center gap-2">
              <span className="text-[9px] font-black text-cloudo-muted uppercase tracking-tighter">From</span>
              <input
                type="date"
            ...
            <div className="flex items-center gap-2">
              <span className="text-[9px] font-black text-cloudo-muted uppercase tracking-tighter">To</span>
              <input
                type="date"
                className="input h-7 px-2 text-[10px] font-black uppercase tracking-widest border-cloudo-border/30 focus:border-cloudo-accent/50 w-32 bg-cloudo-panel/50 [color-scheme:dark]"
                value={toDate}
        ...
        <div className="max-w-[1400px] mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <StatSmall
              ...
              onClick={() => setActionTypeFilter(actionTypeFilter === 'mutation' ? 'all' : 'mutation')}
            ...
            <StatSmall
              ...
              onClick={() => setActionTypeFilter(actionTypeFilter === 'destruction' ? 'all' : 'destruction')}
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 px-2 py-4 border-t border-cloudo-border/30 bg-cloudo-panel/30">
              <div className="flex items-center gap-4">
                ...
                <span className="text-[10px] font-black text-cloudo-muted uppercase tracking-widest">
                  Showing <span className="text-cloudo-text">{(currentPage - 1) * pageSize + 1}</span> to <span className="text-cloudo-text">{Math.min(currentPage * pageSize, filteredLogs.length)}</span> of <span className="text-cloudo-text">{filteredLogs.length}</span>
                </span>
              </div>
...
function StatSmall({ title, value, icon, label, color = "text-cloudo-text", onClick, isActive }: { title: string, value: number, icon: React.ReactNode, label: string, color?: string, onClick?: () => void, isActive?: boolean }) {
  return (
    <div
      onClick={onClick}
    ...
    >
      <div className={`absolute top-0 left-0 w-[2px] h-full transition-colors ${isActive ? 'bg-cloudo-accent' : 'bg-cloudo-err/10'}`} />
      ...
      <div className={`p-2.5 border text-lg shrink-0 transition-colors ${isActive ? 'bg-cloudo-accent/20 border-cloudo-accent' : 'bg-cloudo-err/10 border-cloudo-border opacity-70'}`}>
        <div className={`${isActive ? 'text-white' : 'text-cloudo-err'} w-5 h-5 flex items-center justify-center`}>
          {icon}
        </div>
      </div>
    </div>
