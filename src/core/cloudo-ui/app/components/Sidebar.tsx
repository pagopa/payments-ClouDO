    ...
    >
          ...
          {navigation.filter(item => !item.adminOnly).map((item) => {
            ...
            return (
              ...
              >
                ...
                {!collapsed && (
                  <span className={`text-sm font-bold uppercase tracking-[0.2em] flex-1 ${isActive ? 'text-cloudo-text' : ''}`}>
                    {item.name}
                  </span>
                )}
                {isActive && (
      ...
      <div className="p-4 border-t border-cloudo-border bg-cloudo-accent/5 space-y-2">
        {toggleTheme && (
          <button
            onClick={toggleTheme}
            className={`flex items-center gap-3 w-full px-3 py-2 transition-all border border-cloudo-border text-cloudo-muted hover:bg-cloudo-accent/10 hover:text-cloudo-accent ${collapsed ? 'justify-center px-0' : ''}`}
